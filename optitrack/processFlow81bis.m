...
%% Optic-flow estimated direction to z<0 hemisphere
for i=1:length(CamZ),
    if(CamZ(i)>0),
        CamX(i) = -CamX(i);
        CamY(i) = -CamY(i);
...
save('flow_81_cam_pro_test.mat', 'CamX', 'CamY', 'CamZ', 'GyroX', 'GyroY', 'GyroZ');
...
%% Motion capture direction to z<0 hemisphere
for i=1:length(dirY),
    if(dirY(i)>0),
        dirX(i) = -dirX(i);
        dirY(i) = -dirY(i);
...
save('flow_81_200fps_pro_test.mat', 'velX', 'velY', 'velZ', 'dirX', 'dirY', 'dirZ', 'rateX', 'rateY', 'rateZ', 'Time', 'ErrorMarkers');

%% Load full data
close all
clear all
load('flow_81_cam_pro_test.mat');
load('flow_81_200fps_pro_test.mat');
...
%% Synchronize direction estimates
close all;

% crop data and synchronize
CamX_ = abs(CamX(delay:length(CamX))');
CamY_ = abs(CamY(delay:length(CamY))');
CamZ_ = abs(CamZ(delay:length(CamZ))');
GyroX_ = GyroX(delay:length(GyroX))';
GyroY_ = GyroY(delay:length(GyroY))';
GyroZ_ = GyroZ(delay:length(GyroZ))';
dirX_ = abs(dirX(1:length(CamX_)));
dirY_ = abs(dirY(1:length(CamY_)));
dirZ_ = abs(dirZ(1:length(CamZ_)));
...
close all
% direction estimate along X (absolute value)
figure;
hold on
h1 = plot(abs(dirZ));
h2 = plot(abs(CamX));
...
% direction estimate along Y (absolute value)
figure;
hold on
h1 = plot(abs(dirX));
h2 = plot(abs(CamY));
...
% direction estimate along Z (absolute value)
figure;
hold on
h1 = plot(abs(dirY));
h2 = plot(abs(CamZ));
...
% compute error
accuracy_ = abs(dot(dir_gt', dir_est'));

% compute statistics
acc_mean = mean(accuracy_);
acc_std  = std(accuracy_);
