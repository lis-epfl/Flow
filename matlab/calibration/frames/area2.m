function [fillhandle,msg]=area2(xpoints,upper,lower,color,edge,transparency,parent)
%USAGE: [fillhandle,msg]=area2(xpoints,upper,lower,color,edge,transparency)
...
%edge  = the color around the edge of the filled area (use 'none' for no edge)
...
%     grid on
%     legend('Datr')
if nargin<7;parent=0;end     %default is to add to current plot
...
if length(upper)==length(lower) && length(lower)==length(xpoints)
    ...
    if transparency == 1
        fill(xpoints,filled,color,'EdgeColor',edge);%,'FaceAlpha',transparency); %,'EdgeAlpha',transparency);%plot the data
    else
        fill(xpoints,filled,color,'EdgeColor',edge,'FaceAlpha',transparency); %,'EdgeAlpha',transparency);%plot the data
    end
